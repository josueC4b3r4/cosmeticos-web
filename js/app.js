 ...
 * js/app.js
 *
 * Carga los productos desde un JSON, construye el grid, implementa búsqueda y filtrado,
 * muestra un modal de vista rápida, incluye botón “voler arriba” y anima las tarjetas.
 */
...
// Construye la URL para compartir un producto (incluye hash con el id)
...
function render(list) {
  ...
  for (const p of list) {
    ...
    const hasDiscount = typeof p.precio_original === 'number' && typeof p.precio === 'number' && p.precio_original > p.precio;
    const isAgotado  = p.agotado === true;

    const card = document.createElement('article');
    card.className = `relative bg-white rounded-lg shadow-md overflow-hidden transform transition-opacity duration-500 ease-out opacity-0 translate-y-4 hover:shadow-xl`;

    if (isAgotado) card.classList.add('grayscale','opacity-50','cursor-not-allowed');
    ...
    card.innerHTML = `
      ...
      <div class="p-4 flex flex-col justify-between h-44">
        ...
        <div class="mt-4">
          ${hasDiscount
            ? `<div class="flex items-baseline space-x-2"><span class="text-gray-400 line-through text-sm">${moneyMXN(p.precio_original)}</span><span class="text-lg font-bold text-red-600">${moneyMXN(p.precio)}</span></div>`
      ...
      <div class="p-4 flex space-x-2 border-t">
        ...
        <a class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-3 py-2 rounded-md" target="_blank" rel="noopener" href="${waShareLink(p,urlProducto)}">Compartir</a>
    ...
    img.addEventListener('error', () => {
      img.onerror = null;
      img.src = 'placeholder_light_gray_block.png'; // imagen de relleno si no existe
    });
...
function applyFilters() {
  ...
  const filtered = productos.filter(p => {
    const matchesText = p.nombre.toLowerCase().includes(q) || p.descripcion.toLowerCase().includes(q) || p.categoria.toLowerCase().includes(q);
...
function openModal(p) {
  ...
  if (typeof p.precio_original === 'number' && p.precio_original > p.precio) {
    modalPrice.innerHTML = `<span class="text-gray-400 line-through mr-2">${moneyMXN(p.precio_original)}</span><span class="text-lg font-bold text-red-600">${moneyMXN(p.precio)}</span>`;
  } else {
...
async function init() {
  try {
    ...
    console.error(err);
    grid.innerHTML = '<p class="col-span-full text-center text-red-600">Error cargando productos. Revisa consola.</p>';
  }
}
